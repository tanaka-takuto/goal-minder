...
type accountR struct {
	AccountPassword *AccountPassword `boil:"AccountPassword" json:"AccountPassword" toml:"AccountPassword" yaml:"AccountPassword"`
}
...
func (r *accountR) GetAccountPassword() *AccountPassword {
	if r == nil {
		return nil
	}
...
var (
	accountAllColumns            = []string{"id", "name", "email", "created_at", "updated_at"}
...
// AccountPassword pointed to by the foreign key.
func (o *Account) AccountPassword(mods ...qm.QueryMod) accountPasswordQuery {
	...
	return AccountPasswords(queryMods...)
}

// LoadAccountPassword allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-1 relationship.
func (accountL) LoadAccountPassword(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAccount interface{}, mods queries.Applicator) error {
	var slice []*Account
	var object *Account
	...
	query := NewQuery(
		qm.From(`account_password`),
		qm.WhereIn(`account_password.account_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load AccountPassword")
	}

	var resultSlice []*AccountPassword
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice AccountPassword")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for account_password")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for account_password")
	}
	...
	if singular {
		...
		object.R.AccountPassword = foreign
		if foreign.R == nil {
			foreign.R = &accountPasswordR{}
		}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.ID == foreign.AccountID {
				local.R.AccountPassword = foreign
				if foreign.R == nil {
					foreign.R = &accountPasswordR{}
				}
...
// SetAccountPassword of the account to the related item.
// Sets o.R.AccountPassword to related.
// Adds o to related.R.Account.
func (o *Account) SetAccountPassword(ctx context.Context, exec boil.ContextExecutor, insert bool, related *AccountPassword) error {
	var err error
	...
	if insert {
		...
		values := []interface{}{o.ID, related.AccountID}
	...
	if o.R == nil {
		o.R = &accountR{
			AccountPassword: related,
		}
	} else {
		o.R.AccountPassword = related
	}

	if related.R == nil {
		related.R = &accountPasswordR{
			Account: o,
		}
	} else {
		related.R.Account = o
